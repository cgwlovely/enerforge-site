...
### Projects is project screens only (2026-09-16)

The market-reconstruction group (the mine-electrification study) was removed from the Projects page, the
Projects menu and the footer; that study lives on the Research page only. Projects = single-site screens
(Remote Mining Asset, the illustrative EaaS screen) plus what a screen delivers. The case renderer no longer
emits a sector group. Every page's header and footer are regenerated by `apply_chrome.py`; a scan of all 271
pages found the same chrome on the 256 pages that carry one (the other 15 are redirects and the three
...
### Second language audit applied (2026-09-22, round 20)

A second external pass listed what survived round 19. Fixed immediately: an unclosed `<span>` in the
onsite-abatement recommendation box and a missing `</span>` in the diesel research card, plus every
remaining inconsistency in cash flow, onsite, percentage points a year and "source and confidence rating"
(replacing "evidence grade"). A markup audit across every page found 21 more inline tags that the
tolerant replacement script had swallowed in round 19, and three layout `<div>`s that were never closed;
all are now balanced, and the Pre-DD page's empty closing section has real copy.

Five pages were rewritten rather than copy-edited. The capital-constraint note lost its financing
slogans. The baseline-floor article was retitled "A lower Safeguard threshold would also tighten
baselines for existing facilities" and reduced to about 2,150 words by moving the gold power-station
ownership evidence to the appendix; a short section and a transition sentence remain in the article, so
the ACCU-limits argument still appears where it belongs. The locational-pricing note now separates the
British source material from the Australian inference with a comparison table. The customer-savings
article cut the demand-charge explainer to one example. The weak-grid note went from seven sections to
four: load and reliability, supply options and connection risk, mine life and contract term, and the
commercial boundary with the feasibility decision.

Atlas terminology: "settings in force" is now "selected settings", "residual" is "remaining difference",
"position" is "amount above or below baseline", the emissions reduction contribution is defined on first
use, the iso-target curve is explained before the term is used, and the map legend is a list rather than
one long sentence. Abbreviations (POI, GPS, PCS, SCADA, SOC, DSCR, RTB) are spelled out on first use or
removed. Claim-type and unit codes now show reader labels with the code in brackets, generated by
`scripts/build_api.py` so the next build keeps them.
